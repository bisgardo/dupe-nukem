...
func dirEnsurePath(parent *Dir, path string) *Dir {
	if path != "." {
		for _, p := range strings.Split(path, string(os.PathSeparator)) {
			...
			if child == nil {
				child = NewDir(p)
				// Ordering is maintained by dir entries being constructed in order.
...
func (g *nodeGen) file(path string) *rapid.Generator[file] {
	return rapid.Custom(func(t *rapid.T) file {
		...
		skipped := g.skipNames != nil && rapid.Bool().Draw(t, "skipped")
...
func (g *nodeGen) dir(path string) *rapid.Generator[dir] {
	return rapid.OneOf[dir](
		rapid.Custom(func(t *rapid.T) dir {
			res := make(dir)
			paths := pathsGen(5, 5).Draw(t, "paths")
			sort.Strings(paths) // maintain ordering constraint down the line
			for _, p := range paths {
				n := g.node(filepath.Join(path, p)).Draw(t, "node")
				isSkipped := false
				if n, ok := n.(dirExt); ok {
					isSkipped = n.skipped
				}
				...
				if isSkipped {
					// HACK: Ensure that skipped nodes don't overlap with non-skipped ones.
					// There's just a single global namespace for skipping,
					// and any dir (which may be anywhere within a nested path)
					// might match any file that gets skipped now or later.
					// Sorting that shit out without this is very difficult.
					p += "+skip" // + is never generated by 'pathsGen'
					g.skipNames[filepath.Base(p)] = struct{}{}
				}
				res[p] = n
			}
			return res
...
func Test__scan_property(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		if rapid.Bool().Draw(t, "useCache") {
			g.cache = NewDir(rootPath)
		}
		shouldSkip := NoSkip
		if rapid.Bool().Draw(t, "useSkip") {
			g.skipNames = make(map[string]struct{})
			shouldSkip = SkipNameSet(g.skipNames)
		}
		root := g.dir("").Draw(t, "root")
		root.writeTestdata(t, rootPath)
		want := simulateScan(root, rootPath)
		// Note that cache constraints are not actually being checked by this function -
		// that happens in main.Scan, which is where this function gets called from with the CLI.
		// So it would be more accurate to call that function instead. But that function is in 'main',
		// so we'd have to move this stuff there, meaning that 'node' would have to get exported, ...
		res, err := Run(rootPath, shouldSkip, g.cache)
		require.NoError(t, err)
		res.assertEqual(t, want)
	})
}
