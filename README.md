...
```shell
dupe-nukem match --source <scan-file> [--target <scan-file>]*
```

Match files in the source directory against all target directories using the provided scan files.
If no target directory files are provided, the source file is used as a single target file to locate matches internally in the source directory.
The command outputs a match file which links the hashes of all matched files in the source directory
to the corresponding paths in the target directories.

A match on path `<path>` in the `<n>`'th target directory (counting from 1) is reported as `<n>:<path>`.

All `<scan-file>`s are the scan files generated by the `scan` command
